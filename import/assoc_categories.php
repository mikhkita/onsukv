...
function getCode($code, $index = 0){
	...
	$rsSections = CIBlockSection::GetList(array(),array('IBLOCK_ID' => 1, '=CODE' => $findCode));
...
foreach ($result as $element) { //element - вся информация о элементе
		...
		if(!empty($param_bitrix)){
			if($param_bitrix["TYPE"] == "field"){
				if($param_bitrix["NAME"] == "NAME"){
					$arLoadSectionArray["CODE"] = getCode(str2url(str_replace($replace_symbols, array("", "", '"'), $element[$param])));
					// continue;
				}
	...
	$arLoadSectionArray["PROPERTY_VALUES"] = $PROP;
