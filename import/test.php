...
foreach ($values as $value) {
	if( !isset( $similar[ $value[0] ] ) ){
		$similar[ $value[0] ] = array();
	}
	array_push($similar[ $value[0] ], $value[1]);
}
...
if (file_exists($xmlFileName)) {
	...
	$property_enums = CIBlockPropertyEnum::GetList(Array("DEF"=>"DESC", "SORT"=>"ASC"), Array("IBLOCK_ID"=>1, "CODE"=>"COUNTRY"));
	$countries = array();
	while($enum_fields = $property_enums->GetNext()){
		$countries[ $enum_fields["VALUE"] ] = $enum_fields["ID"];
	}

	$property_enums = CIBlockPropertyEnum::GetList(Array("DEF"=>"DESC", "SORT"=>"ASC"), Array("IBLOCK_ID"=>1, "CODE"=>"MANUFACTURER"));
	$suppliers = array();
	while($enum_fields = $property_enums->GetNext()){
		$suppliers[ $enum_fields["VALUE"] ] = $enum_fields["ID"];
	}
	...
	foreach ($objects as $key => $object) {
		...
		if( isset($object["country"]) && $object["country"] != "" ){
			$object["country"] = trim($object["country"]);
			if( !isset($countries[ $object["country"] ]) ){
				$ibpenum = new CIBlockPropertyEnum;
				if($PropID = $ibpenum->Add(Array('PROPERTY_ID' => 2, 'VALUE' => $object["country"]))){
					$countries[ $object["country"] ] = $PropID;
				}else{
					file_put_contents("test.txt", "Не удалось добавить свойство: ".$object["country"]."\n", FILE_APPEND);
				}
			}

			$PROP["COUNTRY"] = $countries[ $object["country"] ];
		}
		...
		if( isset($object["supplier"]) && $object["supplier"] != "" ){
			$object["supplier"] = trim($object["supplier"]);
			if( !isset($suppliers[ $object["supplier"] ]) ){
				$ibpenum = new CIBlockPropertyEnum;
				if($PropID = $ibpenum->Add(Array('PROPERTY_ID' => 6, 'VALUE' => $object["supplier"]))){
					$suppliers[ $object["supplier"] ] = $PropID;
				}else{
					file_put_contents("test.txt", "Не удалось добавить свойство: ".$object["supplier"]."\n", FILE_APPEND);
				}
			}

			$PROP["MANUFACTURER"] = $suppliers[ $object["supplier"] ];
		}
		...
		if( isset($result[ $object["id"] ]) ){
			$ext = $result[ $object["id"] ];

			if( isset($ext["IS_WEIGHT"]) ){
				$PROP["IS_WEIGHT"] = ($ext["IS_WEIGHT"] == 1) ? 2 : "";
			}

			if( isset($ext["STATUS"]) ){
				$arLoadProductArray["ACTIVE"] = (intval($ext["STATUS"]) == 1) ? "Y" : "N";
			}
		}

		if( isset($similar[ $object["id"] ]) ){
			$PROP["SIMILAR"] = $similar[ $object["id"] ];
		}
