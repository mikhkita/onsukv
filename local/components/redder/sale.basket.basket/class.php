...
class CBitrixBasketComponent extends CBitrixComponent
{
	...
	protected function initializeBasketOrderIfNotExists(Sale\Basket $basket)
	{
		if (!$basket->getOrder())
		{
			...
			// Скидка 2% за 5 отзывов в течение недели
			$countReviews = getParam("REVIEWS_COUNT_TO_DISCOUNT");
			if( getReviewCount() >= intval($countReviews["VALUE"]) ){
				Sale\DiscountCouponsManager::add("AJSD123uyiashjdk8932ioda");
			}else{
				Sale\DiscountCouponsManager::delete("AJSD123uyiashjdk8932ioda");
			}
			// Скидка 2% за 5 отзывов в течение недели
