	...
	BX.Sale.BasketComponent = {
		...
		fillTotalBlocks: function()
		{
			...
			if (totalNodes && totalNodes.length)
			{
				...
				if (totalTemplate)
				{
					if( document.getElementById('b-delivery-price-input').value != 0 ){
						this.result.TOTAL_RENDER_DATA.DELIVERY_PRICE = document.getElementById('b-delivery-price-input').value;
					}
		...
		checkMinPrice: function(totalData){
			if( document.getElementById('b-basket-checkout-button') && typeof totalData.PRICE_WITHOUT_DELIVERY != "undefined" ){
